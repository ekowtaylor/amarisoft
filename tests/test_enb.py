...
class TestStatsOverride:
    ...
    def test_stats_with_rf(self, mock_client, enb):
        enb.stats(rf=True)
        mock_client.send.assert_called_once_with({"message": "stats", "rf": True})
...
class TestRf:
    ...
    def test_rf_tx_gain(self, mock_client, enb):
        enb.rf(tx_gain=20.0)
        mock_client.send.assert_called_once_with({"message": "rf", "tx_gain": 20.0})

    def test_rf_rx_gain(self, mock_client, enb):
        enb.rf(rx_gain=30.0)
        mock_client.send.assert_called_once_with({"message": "rf", "rx_gain": 30.0})

    def test_rf_rx_agc(self, mock_client, enb):
        enb.rf(rx_agc=True)
        mock_client.send.assert_called_once_with({"message": "rf", "rx_agc": True})
