"""Logging and diagnostics collection for Amarisoft Callbox test runs.
...
class LogEntry:
    ...
    # Level mapping from Amarisoft integer codes to strings
    LEVEL_MAP = {
        0: "NONE",
        1: "ERROR",
        2: "WARNING",
...
class LogCollector:
    """Collects logs from Callbox services.
    ...
    def collect_once(
        ...
        Args:
            services: List of services to collect from. Default: all connected.
        ...
        """
        if services is None:
            services = [s for s, connected in self.callbox.status.items() if connected]
    ...
    def _parse_log_entry(self, log: dict[str, Any], service: str) -> LogEntry:
        ...
        # Handle 'data' field which contains message array
        ...
        # Convert timestamp - can be unix ms or string
...
class TestSession:
    """Context manager for end-to-end test runs with automatic log capture.
    ...
    def __init__(
        ...
        Args:
            ...
            folder_prefix: Optional prefix for the output folder name.
                          If not provided, uses `name`. Timestamp is always appended.
                          Example: folder_prefix="volte_e2e" -> "volte_e2e_20260217_163000"
        ...
        self.folder_prefix = folder_prefix  # If None, uses name
    ...
    def _log_callback(self, entry: LogEntry) -> None:
        ...
        # Could be used for real-time log streaming
        pass

    def _capture_initial_state(self) -> None:
        """Capture initial configuration and stats."""
        for service in ["enb", "mme", "ims"]:
            if self.callbox.status.get(service):
                try:
    ...
    def _collect_error_diagnostics(self) -> None:
        ...
        # Get all current UEs
        try:
            if self.callbox.status.get("enb"):
                self._error_ues_enb = self.callbox.enb.ue_get()
            if self.callbox.status.get("mme"):
    ...
    def export_diagnostics(self, output_dir: Path | None = None) -> Path:
        ...
        session_info = {
            ...
            "callbox_host": self.callbox.host,
            "services_connected": self.callbox.status,
        }
    ...
    def _generate_summary(self) -> str:
        ...
        for service in ["enb", "mme", "ims"]:
            service_logs = self._collector.filter_logs(service=service)
            if service_logs:
                error_count = len([l for l in service_logs if l.level.upper() == "ERROR"])
                warn_count = len([l for l in service_logs if l.level.upper() in ("WARN", "WARNING")])
...
def enable_file_logging(
    log_file: str | Path = "amarisoft.log",
    level: int = logging.DEBUG,
    format_str: str = "%(asctime)s [%(name)s] %(levelname)s: %(message)s",
) -> logging.FileHandler:
    """Enable logging to a file for the amarisoft package.
    ...
    # Add to amarisoft logger
    amari_logger = logging.getLogger("amarisoft")
    amari_logger.addHandler(handler)
    amari_logger.setLevel(min(amari_logger.level or logging.DEBUG, level))
